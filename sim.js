...
(function (root) {
  ...
  //    お化け位置/間合い/クリア窓/穴 を書き換える。step() はこれらを参照するだけ。
  // =====================================================================
  ...
  // =====================================================================
  // ★ cmd_551 環境ギミック（アクティブな局面の隠れ場所/思い出の品。applyBeat が GAP と同じ流儀で載せ替える）
  //   step() はこれらを参照値として読む。空なら no-op。
  // =====================================================================
  var HIDE_CELLS = [];               // 隠れ場所マス（このマスで しゃがむ と身を隠せる）。空=ギミック無し
  var MEMENTOS = [];                 // 思い出の品 [{cell, kind, correct}]。複数候補（誤りは囮）。空=ギミック無し
  var MEMENTO_RANGE = 14;            // 「品の近く」とみなす world 距離（局面で上書き可）
  var MEMENTO_TRUST_MUL = 1.9;       // 正しい品の射程内で しゃがみ信頼を育てる際の trust 増加倍率（局面で上書き可）

  // 隠れ場所マスの上に居るか（grounded 前提・現在セルで判定。決定論）
  function isHideCell(x) {
    if (!HIDE_CELLS.length) return false;
    var c = xToCell(x);
    for (var i = 0; i < HIDE_CELLS.length; i++) if (HIDE_CELLS[i] === c) return true;
    return false;
  }
  // 正しい思い出の品の射程内に居るか（誤った品は false＝囮。決定論）
  function nearCorrectMemento(x) {
    if (!MEMENTOS.length) return false;
    for (var i = 0; i < MEMENTOS.length; i++) {
      var m = MEMENTOS[i];
      if (m.correct && Math.abs(cellToX(m.cell) - x) <= MEMENTO_RANGE) return true;
    }
    return false;
  }
  ...
  var STAGES = [
    ...
    // ===================================================================
    // ⑥ ★ 環境ギミック（隠れ場所＋思い出の品）showcase
    //    4局面: travel(穴ジャンプ) → ghost A 過敏/隠れ場所 → travel(穴ジャンプ) → ghost B 本命/思い出の品(final)
    //    ・beat1: 過敏な“こわがり”。物陰マス(hideCells)でしゃがむと視線を切ってやり過ごし、近くで信頼を育てられる。
    //    ・beat3: 本命“さみしがり”。複数の思い出の品から、会話ヒントの“正しい品”のそばでしゃがむと早く心を開く。
    //    隠れ場所＝『しゃがむ』の二役化、思い出の品＝『どこでしゃがむ＝間合い/待つ』の意味を文脈で深める（語彙は5種のまま）。
    // ===================================================================
    {
      id: 'memento',
      name: '⑥ 想い出の縁日（ギミック）',
      blurb: '★多局面＋環境ギミック。物陰に隠れて過敏な子をやり過ごし、思い出の品のそばで本命の心を開く。会話のヒントから「どこに隠れ」「どの品が効く」かを推理。語彙は5種のまま。',
      geom: { xMax: 204, playerCell: 1 },
      intro: [
        { who: '案内のおばけ', text: '今夜の縁日にも、二人のおばけが待ってる。' },
        { who: '案内のおばけ', text: '手前の子は、ひどい怖がり。じっと見られると固まってしまう。…でも“物陰”でなら、そっと近づけるかも。' },
        { who: '案内のおばけ', text: '奥の子は、さみしがり。生きてた頃、リンゴ飴をいつも握りしめてたんだって。' },
        { who: 'あなた', text: '（物陰に隠れて手前の子をやり過ごし…奥の子は“好きだった物”のそばで寄り添えばいいのかな）' }
      ],
      beats: [
        // beat0: 橋（穴）をジャンプで渡る。
        { kind: 'travel', goalCell: 5, gap: { cell: 4 },
          banner: '細い橋。むこうの暗がりに、怖がりな子の気配。まずジャンプで渡って。' },
        // beat1: 過敏な“こわがり”＋【隠れ場所】。物陰(cell9=お化けのすぐ手前)でしゃがむと視線を切ってやり過ごせる。
        //   ghost cell10 のすぐ隣 cell9 は本来「深い食い込み」＝普通にしゃがむと警戒が上がり信頼が育たない。
        //   物陰だと penEff=0 ＝ 過敏なお化けの眼前でも警戒が上がらず信頼が育つ＝やり過ごして近づける。
        { kind: 'ghost', ghostCell: 10, gap: null,
          feel: { comfortFar: 30, spaceMargin: 12 },
          win:  { adjacent: 13, trustWin: 58, winWaryGate: 46 },
          name: 'こわがり',
          banner: '“こわがり”：…っ（びくっ）　— じっと見られると、固まっちゃう。',
          clue: 'ふつうに眼前で粘ると警戒MAX。“物陰（草むら）”でしゃがめば視線を切ってそっと寄れる。物陰はどこ？',
          gimmicks: { hideCells: [9] },
          final: false },
        // beat2: もう一つの裂け目をジャンプで渡る。
        { kind: 'travel', goalCell: 13, gap: { cell: 12 },
          banner: '“こわがり”が道をあけてくれた。次の裂け目を跳んで、奥の子へ。' },
        // beat3: 本命“さみしがり”＋【思い出の品】。正しい品(リンゴ飴=cell16)のそばでしゃがむと早く心を開く。final。
        { kind: 'ghost', ghostCell: 18, gap: null,
          feel: { comfortFar: 22, spaceMargin: 8 },
          win:  { adjacent: 12, trustWin: 72, winWaryGate: 42 },
          name: 'さみしがり',
          banner: '“さみしがり”のそばに、いくつかの“思い出の品”が落ちている。',
          clue: '生前ずっと握りしめてた物のそばで寄り添うと、早く心を開く。会話のヒントから“その品”はどれ？',
          gimmicks: { mementos: [ { cell: 14, kind: 'bell', correct: false },
                                   { cell: 16, kind: 'apple', correct: true } ],
                      mementoRange: 9 },
          final: true }
      ],
      // お手本（1本の計画）。tune 済み: beat0渡る→物陰(cell9)で隠れてA→beat2渡る→リンゴ飴(cell16)のそばでB。
      example: [
        'right', 'right', 'jump',                               // beat0: 橋を渡る（~cell5へ）
        'right', 'right', 'right', 'right',                     // beat1: お化け眼前の物陰(cell9)へ
        'crouch', 'crouch', 'crouch', 'crouch',                 // beat1: 物陰でしゃがみ“こわがり”の視線を切って攻略
        'right', 'right', 'jump',                               // beat2: 二つ目の穴を渡る
        'right', 'right', 'right',                              // 本命Bへ：リンゴ飴(cell16)のそばへ
        'crouch', 'crouch', 'crouch', 'crouch'                 // beat3: 正しい品のそばでしゃがんでB攻略
      ]
    }
  ];
  ...
  function loadStage(idx) {
    ...
    // 既定の穴/間合い/クリア窓/ギミックは beat0 を当てておく（draw のフォールバック用）
    applyBeatGeometry(s.beats[0]);
    applyBeatGimmicks(s.beats[0]);   // ★ cmd_551: 描画フォールバック用にギミックも初期化（無指定なら空）
  ...
  function beatMeta(idx) {
    ...
    return s.beats.map(function (b, i) {
      return { index: i, kind: b.kind, name: b.name || null, banner: b.banner || '',
               ...
               gimmicks: b.gimmicks || null };   // ★ cmd_551: 隠れ場所/思い出の品の描画データ（無ければ null）
    });
  }
  ...
  function applyBeatGimmicks(beat) {
    var g = beat.gimmicks || null;
    HIDE_CELLS        = (g && g.hideCells)  ? g.hideCells.slice()  : [];
    MEMENTOS          = (g && g.mementos)   ? g.mementos.map(function (m) { return { cell: m.cell, kind: m.kind, correct: !!m.correct }; }) : [];
    MEMENTO_RANGE     = (g && g.mementoRange != null) ? g.mementoRange : 14;
    MEMENTO_TRUST_MUL = (g && g.mementoTrustMul != null) ? g.mementoTrustMul : 1.9;
  }
  ...
  function step(state, input, dt) {
    ...
    //    nearGood=false となり、以降の式は cmd_549 と完全一致＝ no-op）。いずれも state×位置の純関数＝決定論。
    var hidden = state.crouching && isHideCell(state.playerX);  // 隠れ場所マスでしゃがむ＝物陰に身を隠す（視線を切る）
    var penEff = hidden ? 0 : penetration;                      // 隠れている間は間合いの食い込みを“見られていない”＝0扱い
    var nearGood = nearCorrectMemento(state.playerX);           // 正しい思い出の品の射程内に居るか
    ...
    if (state.crouching) {
      if (still) {
        ...
        // ★ 隠れ場所では penEff=0 → 過敏なお化けの近く（深い食い込み）でも警戒が上がらない＝やり過ごせる
        if (penEff > DEEP_PEN) state.wariness += WARY_DEEP_CROUCH * (penEff - DEEP_PEN) / (1 - DEEP_PEN) * dt;
      } else {
        if (inside) state.wariness += WARY_DEEP_CROUCH * penEff * dt;
    ...
    // ★ 隠れ場所では penEff<DEEP_PEN が成立しやすく、過敏なお化けの近くでも信頼を育てられる（やり過ごして近づく）。
    //   思い出の品: 正しい品の射程内なら trust 増加に memMul を掛ける＝通常より早く心を開く（誤り/無視は memMul=1）。
    if (state.crouching && still && penEff < DEEP_PEN && state.wariness < TRUST_WARY_GATE) {
      var farMul = (dist > VIEW) ? TRUST_FARMUL : 1;
      var memMul = nearGood ? MEMENTO_TRUST_MUL : 1;
      state.trust += TRUST_RATE * farMul * memMul * dt;
    ...
    // ★ 隠れ場所: 物陰のプレイヤーを、過敏なお化けは「詰め寄られている」と感じない（視線を切っている）＝
    //   間合いを詰めても退かない。隠れている間は目標距離を現在距離まで縮め、近くに居させる＝
    //   信頼が育つほど自分から寄ってくる。これが「警戒をやり過ごして近づける」の正体。
    //   隠れ場所マスが無い局面では hidden=false ＝この行は no-op。
    if (hidden) { desiredDist = Math.min(desiredDist, dist); canApproach = true; }
  ...
  //   フレームごとに「今の局面が満たされたか」を判定し、満たされたら次局面へ。
  ...
  function snapshot(st, blockIndex, beatIndex) {
    return {
      ...
      hidden: st.crouching && isHideCell(st.playerX),       // 今しゃがんで物陰に隠れているか
      nearMemento: nearCorrectMemento(st.playerX),          // 正しい思い出の品の射程内に居るか
  ...
  var API = {
    ...
    // ギミックAPI（テスト/描画が現在アクティブな隠れ場所/思い出の品を参照）
    isHideCell: isHideCell, nearCorrectMemento: nearCorrectMemento,
    get HIDE_CELLS() { return HIDE_CELLS.slice(); },
    get MEMENTOS() { return MEMENTOS.map(function (m) { return { cell: m.cell, kind: m.kind, correct: m.correct }; }); },
    get MEMENTO_RANGE() { return MEMENTO_RANGE; },
    get MEMENTO_TRUST_MUL() { return MEMENTO_TRUST_MUL; },
    // テスト専用: 指定ステージ/局面の feel・gimmick・geometry をモジュールへ適用（state社交リセットはしない）。
    // step() の単体検証（隠れ場所/思い出の品が deterministic に効くか）に使う。
    _testApplyBeat: function (stageIdx, beatIdx) {
      loadStage(stageIdx);
      var b = STAGES[clamp(stageIdx | 0, 0, STAGES.length - 1)].beats[beatIdx | 0];
      applyBeatGeometry(b); applyBeatGimmicks(b);
      if (b.kind === 'ghost') applyBeatFeel(b);
      return b;
    },
