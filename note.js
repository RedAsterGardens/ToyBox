...
(function (root) {
  ...
  var PERSONA = {
    ...
    //   品が無い局面（⑤の さみしがり 等）では nearMemento が常に false ＝ mementoGlad は発火せず従来と同一。
    lonely: {
      label: 'さみしがり',
      ...
      facts: {
        ...
        mementoGlad: 'ある “思い出の品” の そばで しゃがむと、いつもより ずっと うれしそうに していた。',
        won:         'となりで よりそって 待ったら、はじめて わらって くれた。'
      },
      hypothesis: '🔎 仮説：この子は ずっと ひとりで さみしいのかも？ ' +
                  'にげずに そばまで 行って、となりで しゃがんで よりそうと いいのかも…？ ' +
                  '“好きだった物”の そばだと、もっと はやく ひらくのかも…？'
    },
    //   ふつうの臆病(timid)は「離れてしゃがむ」が肯定だが、この子は『物陰で視線を切れば近くでも平気』＝逆説。
    //   → timid と別ペルソナにし、隠れ場所由来の事実 hidUsed と それに沿った仮説を持たせる（正解の“どこ”は直書きしない）。
    timidHide: {
      label: 'とても こわがり',
      entry: 'この子は とても こわがりで、じっと 見られていると 体が こわばってしまうみたい。',
      facts: {
        rushed:      '立ったまま 近づくと、見つかって ますます 固まってしまった。',
        closed:      'ぐいぐい 行きすぎて、心を とじてしまった。',
        crouchedFar: 'はなれて しゃがむと、すこしずつ 落ち着いて いった。',
        crouchedNear:'近くで しゃがんでも、見られている あいだは そわそわ していた。',
        hidUsed:     'ものかげ（草むら）で しゃがんでいる あいだは、すぐ そばでも こわがらせずに いられた。',
        won:         '視線を うまく かわして そばに いたら、そっと 心を ひらいて くれた。'
      },
      hypothesis: '🔎 仮説：この子は じっと 見られるのが にがてなのかも？ ' +
                  'ものかげに かくれて 視線を 切ってから そっと 近づくと、心を ひらいて くれるのかも…？'
    },
  ...
  //   どちらも該当ペルソナにしか facts を持たせていないので、無関係なお化けでは記録されない（deriveBeatFacts で p.facts[id] を guard）。
  var FACT_ORDER = ['rushed', 'closed', 'crouchedFar', 'crouchedNear', 'mementoGlad', 'hidUsed', 'won'];
  ...
  //   ★ snapshot のフィールド（crouching/dist/guardR/phase/closed/result/hidden/nearMemento）だけを見る＝物理を再計算しない。
  // =====================================================================
  function deriveBeatFacts(simResult, beatIndex, beatMetaArr) {
    ...
    var sig = { rushed: false, closed: false, crouchedFar: false, crouchedNear: false,
                mementoGlad: false, hidUsed: false, won: false };
    for (var i = 0; i < frames.length; i++) {
      ...
      if (!sn.crouching) {
        ...
        if (sn.nearMemento) sig.mementoGlad = true;
      }
    }

    var facts = [];
    FACT_ORDER.forEach(function (id) {
      // ★cmd_554: p.facts[id] を guard。ギミック事実を持たないペルソナでは undefined を記録しない。
      if (sig[id] && p.facts[id]) facts.push({ id: id, text: p.facts[id] });
    });
