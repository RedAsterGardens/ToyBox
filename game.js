(function () {
  ...
  var sawHidden = false;         // 物陰に隠れた瞬間
  var sawMemento = false;        // 正しい品が効いた瞬間

  //   planDirty: 計画を変えてからまだ「▶最後まで再生」して記録していない＝記録のチャンスがある状態。
  //   freshFacts: 直近の observe で新しく増えた事実ID（お化け別）。ノート描画でハイライトする。
  var planDirty = false;
  var freshFacts = {};
  var elRecordCue = document.getElementById('recordcue');   // 「▶再生で記録できる」誘導（無くても動く）
  var elToast     = document.getElementById('notetoast');   // 「📝記録した！」トースト（無くても動く）
  var elPlayBtn   = document.getElementById('play');
  var toastTimer  = null;
  ...
  // ===================== ステージ表示 =====================
  // 公開版は単一ステージ（想い出の縁日）のみ。選択タブUIは無いので、要素が無ければ何もしない。
  function renderTabs() {
    if (!elTabs) return;
    elTabs.innerHTML = '';
    ...
    if (elBlurb) elBlurb.textContent = S.currentStage().blurb;
  }
  function selectStage(idx) {
    ...
    plan = { blocks: [] }; selected = -1; t = 0; mode = 'plan'; sawClosed = false; sawHidden = false; sawMemento = false; lastBeatSeen = 0;
    if (heroNote) heroNote.reset();           // ★ 別ステージ＝別のお化け。ノートはやり直し
    observedEndForT = -1;
    planDirty = false; freshFacts = {};
    rebuild(); renderTabs(); renderBlocks(); renderBeatbar(); renderBeatInfo(); renderNotePanel(); setBanner(); draw();
    updateRecordCue();
    showIntro(idx);
  }

  function markPlanDirty() { planDirty = plan.blocks.length > 0; updateRecordCue(); }
  ...
  function addBlock(type) {
    ...
    t = 0; rebuild(); renderBlocks(); renderBeatbar(); setBanner(); draw(); markPlanDirty();
  }
  function delBlock() {
  ...
  function clearPlan() {
    ...
    mode = 'plan'; rebuild(); renderBlocks(); renderBeatbar(); setBanner(); draw(); markPlanDirty();
  }
  function loadExample() {
    plan = S.examplePlan(); selected = -1; t = 0; mode = 'plan';
    rebuild(); renderBlocks(); renderBeatbar(); setBanner(); markPlanDirty();
  ...
  function driveAppend(type) {
    ...
    if (t > endT) t = endT;
    renderBlocks(); renderBeatbar(); renderBeatInfo(); setBanner(); draw(); markPlanDirty();
  }
  ...
  function renderNotePanel() {
    ...
    // ★ cmd_554: 記録トリガーの明確化 — 「何をすれば記録が増えるか」を常時はっきり示す。
    html += '<div class="noteguide">🔍 <b>記録のふやし方：</b>いろいろ動かして <b>▶を最後まで再生</b>すると、' +
            'お化けの反応が <b>「事実」</b>として ここに増えるよ。' +
            '<b>立つ／はなれてしゃがむ／物陰／品のそば</b> など、ちがう動きを試すほど 発見が集まる。</div>';
    ...
    ghostBeats.forEach(function (b) {
      ...
      if (facts.length === 0) {
        html += '<div class="noteempty">（まだ 発見なし。動かして ▶再生して 観察してみよう）</div>';
      } else {
        facts.forEach(function (f) {

          var isNew = fresh.indexOf(f.id) >= 0;
          html += '<div class="notefact' + (isNew ? ' fresh' : '') + '">' +
                  (isNew ? '<span class="newtag">📝NEW</span> ' : '・') + esc(f.text) + '</div>';
        });
      }
      html += '</div>';
  ...
  function observeTrial() {
    ...
    beats.forEach(function (b) {
      if (b.kind === 'ghost') before[b.name] = heroNote.factsOf(b.name).map(function (f) { return f.id; });
    });
    ...
    var totalNew = 0;
    beats.forEach(function (b) {
      if (b.kind !== 'ghost') return;
      var prev = {}; (before[b.name] || []).forEach(function (id) { prev[id] = 1; });
      var add = heroNote.factsOf(b.name).map(function (f) { return f.id; }).filter(function (id) { return !prev[id]; });
      if (add.length) { freshFacts[b.name] = add; totalNew += add.length; }
    });
    planDirty = false;                 // ★ この計画はもう記録済み
    renderNotePanel();
    updateRecordCue();
    if (totalNew > 0) {
      showRecordToast(totalNew);       // 📝記録した！（ハイライト＋トースト＋効果音）
      beep();
    } else {
      flashHint('📝 今回の再生では 新しい発見はなかった。別の動き（立つ／はなれてしゃがむ／物陰／品のそば）を試すと 事実が増えるよ。');
    }
  }

  function updateRecordCue() {
    var recordable = planDirty && plan.blocks.length > 0;
    if (elRecordCue) {
      elRecordCue.style.display = recordable ? 'block' : 'none';
      if (recordable) elRecordCue.textContent = '🎬 いまの計画を ▶再生で ノートに記録できる（最後まで再生すると 観察した事実が増える）';
    }
    if (elPlayBtn) elPlayBtn.classList.toggle('cue', recordable);   // ▶ボタンを光らせて記録を促す
  }

  function showRecordToast(n) {
    if (!elToast) { flashHint('📝 ＋' + n + '件 ノートに記録した！'); return; }
    elToast.textContent = '📝 ＋' + n + '件 記録した！';
    elToast.classList.remove('show'); void elToast.offsetWidth;   // アニメ再トリガー
  ...
  function beep() {
    try {
      ...
      o.frequency.setValueAtTime(1320, now + 0.10);   // 上がる2音＝「ピコン♪」
      o.start(now); o.stop(now + 0.24);
    } catch (e) { /* 音が出せない環境は無視（演出はトースト＋ハイライトで担保） */ }
  }
  ...
  function play() {
    ...
    observedEndForT = -1;
    freshFacts = {}; renderNotePanel();
    t = 0; slider.value = 0; mode = 'playing'; lastFrameMs = null; sawClosed = false; sawHidden = false; sawMemento = false; lastBeatSeen = 0; setBanner();
  }
  function pause() { if (mode === 'playing') { mode = 'paused'; setBanner(); } }
  function rewind() { stopAllDrive(); followEnd = false; t = 0; slider.value = 0; mode = 'plan'; sawClosed = false; sawHidden = false; sawMemento = false; lastBeatSeen = 0; setBanner(); renderBlocks(); renderBeatbar(); renderBeatInfo(); draw(); }
  ...
  function tick(now) {
    if (mode === 'playing' && sim) {
      ...
      if (snap) {
        ...
        if (snap.hidden && !sawHidden) { sawHidden = true; flashHint('🌿 物陰に隠れた！　視線を切ると、過敏な子の眼前でも こわがらせずに そっと近づける。（この観察がノートに記録される）'); }
        if (snap.nearMemento && snap.crouching && !sawMemento) { sawMemento = true; flashHint('♪ 思い出の品が効いてる！　この子の“好きだった物”のそば＝早く心を開く。（この観察がノートに記録される）'); }
  ...
  // 現在局面のギミックデータ（隠れ場所/思い出の品）を beatMeta から取り出す
  function activeGimmicks(snap) {
    var b = beats[snap.beatIndex];
    return (b && b.gimmicks) ? b.gimmicks : null;
  }
  // 隠れ場所（物陰/草むら）。プレイヤーがそこでしゃがんで隠れていると明るく光る。
  function drawHideSpots(snap) {
    var g = activeGimmicks(snap);
    if (!g || !g.hideCells) return;
    var pc = S.xToCell(snap.playerX);
    g.hideCells.forEach(function (c) {
      var cx = px(S.cellToX(c));
      var onIt = !!snap.hidden && pc === c;
      // 草むらの茂み（3つの半円）。隠れ中は鮮やかに。
      ctx.fillStyle = onIt ? 'rgba(124,222,140,0.62)' : 'rgba(96,166,116,0.42)';
      for (var k = -1; k <= 1; k++) {
        ctx.beginPath();
        ctx.arc(cx + k * CELL_PX * 0.24, GROUND_PY - 5, CELL_PX * 0.21, Math.PI, 0);
        ctx.fill();
      }
      // 縦の草
      ctx.strokeStyle = onIt ? 'rgba(180,245,195,0.9)' : 'rgba(150,200,165,0.7)';
      ctx.lineWidth = 1.5;
      for (var j = -2; j <= 2; j++) {
        var gx = cx + j * CELL_PX * 0.14;
        ctx.beginPath(); ctx.moveTo(gx, GROUND_PY - 4); ctx.lineTo(gx + 1.5, GROUND_PY - 13); ctx.stroke();
      }
      ctx.fillStyle = onIt ? '#cffdd9' : 'rgba(176,214,186,0.85)';
      ctx.font = '10px sans-serif'; ctx.textAlign = 'center';
      ctx.fillText(onIt ? '🌿 隠れ中' : '🌿 物陰', cx, GROUND_PY - 20);
    });
  }

  var MEMENTO_ICON = { apple: '🍎', bell: '🔔', ribbon: '🎀', candy: '🍬', flower: '🌼' };
  var MEMENTO_LABEL = { apple: 'リンゴ飴', bell: '鈴', ribbon: 'リボン', candy: 'あめ', flower: '花' };
  // 思い出の品（複数候補）。どれが“正解”かは描き分けない（推理）。
  // 正しい品のそばでしゃがんで効いている時だけ淡い光で「効いてる」手応えを返す（実験で察する）。
  function drawMementos(snap) {
    var g = activeGimmicks(snap);
    if (!g || !g.mementos) return;
    var range = (g.mementoRange != null) ? g.mementoRange : 14;
    g.mementos.forEach(function (m) {
      var cx = px(S.cellToX(m.cell));
      var working = !!snap.nearMemento && !!snap.crouching && m.correct &&
                    Math.abs(S.cellToX(m.cell) - snap.playerX) <= range;
      if (working) {
        ctx.fillStyle = 'rgba(255,213,79,0.28)';
        ctx.beginPath(); ctx.arc(cx, GROUND_PY - 10, CELL_PX * 0.55, 0, 7); ctx.fill();
      }
      // 台座
      ctx.fillStyle = 'rgba(255,255,255,0.10)';
      ctx.fillRect(cx - CELL_PX * 0.18, GROUND_PY - 4, CELL_PX * 0.36, 4);
      // アイコン＋名札（“何の品か”は推理に必要なので見せる。“正解”は明示しない）
      ctx.font = Math.max(14, (CELL_PX * 0.5) | 0) + 'px serif'; ctx.textAlign = 'center';
      ctx.fillText(MEMENTO_ICON[m.kind] || '🎁', cx, GROUND_PY - 8);
      ctx.fillStyle = working ? '#ffe082' : 'rgba(220,228,236,0.78)';
      ctx.font = '10px sans-serif';
      ctx.fillText(MEMENTO_LABEL[m.kind] || m.kind, cx, GROUND_PY - 24);
      if (working) {
        ctx.fillStyle = '#ffd54f'; ctx.font = 'bold 11px sans-serif';
        ctx.fillText('♪', cx + CELL_PX * 0.28, GROUND_PY - 28 + Math.sin(t * 6) * 2);
      }
    });
  }
  ...
  function drawPlayer(snap) {
    ...
    if (snap.hidden) drawMark(x, headY - headR - 6, '🌿', '#9ccc65');   // ★ 物陰に隠れている手応え
    else if (crouch) drawMark(x, headY - headR - 6, '…', '#80cbc4');
  }
  ...
  // ===================== ボタン配線 =====================
  // （公開版はカード式UIを撤去。data-add / moveL / moveR / del が存在しない場合は配線をスキップ）
  ...
  var elMoveL = document.getElementById('moveL'); if (elMoveL) elMoveL.onclick = function () { moveBlock(-1); };
  var elMoveR = document.getElementById('moveR'); if (elMoveR) elMoveR.onclick = function () { moveBlock(+1); };
  var elDel   = document.getElementById('del');   if (elDel)   elDel.onclick   = delBlock;
  ...
  // ===================== ドライブ配線 =====================
  // （公開版は入力方式トグルを撤去。要素が無ければスキップ＝常に直接ドライブ式）
  ...
  // ===================== 起動 =====================
  // 公開版は「想い出の縁日」1ステージのみ。STAGE_INDEX に固定して起動する。
  var STAGE_INDEX = S.stageList().length - 1;   // 最後のステージ＝⑥ 想い出の縁日（ギミック）
  S.loadStage(STAGE_INDEX); applyStageView();
  beats = S.beatMeta(STAGE_INDEX);
  rebuild(); renderTabs(); renderBlocks(); renderBeatbar(); renderBeatInfo(); renderNotePanel(); setBanner(); draw();
  setInputMode('drive');    // 公開版は直接ドライブ式（WASD）のみ
  updateRecordCue();        // 記録誘導の初期表示
  showIntro(STAGE_INDEX);
  requestAnimationFrame(tick);
})();
